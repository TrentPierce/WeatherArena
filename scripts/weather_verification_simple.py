#!/usr/bin/env python3
"""
WeatherArena Verification Pipeline - Simplified HTTP Version
Zero-storage architecture: Calculate errors immediately, discard raw data.

Usage:
    python weather_verification_simple.py
...
def insert_verification_result(
    ...
    try:
        ...
        # Check if the request was successful (status 201 for insert)
        # Supabase returns empty array on successful insert
        if result is not None:  # Request succeeded, even if response is empty
            logger.info(f"Inserted verification result for {location_code}: error={error_value}°C")
            return True
        else:
            logger.error(f"Failed to insert verification result")
            return False
...
def cleanup_old_records(config: Dict[str, Any], days: int = 7) -> Optional[int]:
    ...
    try:
        ...
        if result is not None:
            ...
            logger.error(f"Failed to cleanup old records")
            return None
...
def update_model_ranking(config: Dict[str, Any], model_name: str, error_value: float) -> bool:
    ...
    try:
        ...
        if existing and len(existing) > 0:
            ...
            # For updates, we need to use the primary key (id)
            update_data = {
                'elo_score': new_elo,
            ...
            if result:
                ...
                logger.error(f"Failed to update ranking for {model_name}")
                return False
        else:
            ...
            if result is not None:
                ...
                logger.error(f"Failed to create ranking for {model_name}")
                return False
